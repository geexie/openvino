...
void jit_equal_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    Vmm vmm_aux0 = Vmm(aux_vec_idxs[0]);
    Vmm vmm_aux1 = Vmm(aux_vec_idxs[2]);
...
: jit_emitter(host, host_isa, node, exec_prc) {
    remark(10) << "PowerEmitter: " << node->get_friendly_name() << " " << node->get_type_info().name << std::endl;
    ...
    power = ngraph::as_type_ptr<ngraph::op::Scalar>(parent)->get_data_ptr<float>()[0];
    std::cout << "order = " << power << std::endl;
