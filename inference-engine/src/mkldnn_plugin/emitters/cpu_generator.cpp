...
MKLDNNPlugin::CPUGenerator::CPUGenerator() : h(new jit_snippet()), isa(dnnl::impl::cpu::x64::avx2) {
    // data movement
    ...
    jitters[ngraph::opset1::Clamp().get_type_info()] = CREATE_EMITTER(MKLDNNPlugin::jit_clamp_emitter);
    ...
    jitters[ngraph::opset1::Negative().get_type_info()] = CREATE_EMITTER(MKLDNNPlugin::jit_negative_emitter);
...
auto get_lanes(dnnl::impl::cpu::x64::cpu_isa_t isa) -> size_t {
    switch (isa) {
...
code MKLDNNPlugin::CPUGenerator::generate(std::shared_ptr<ngraph::Function>& f) const {
    ...
    if (nparams > 7) {
        throw ngraph::ngraph_error(std::string("snippet signature should not exceed 7 arguments. got") + std::to_string(nparams));
    }
    ...
    for (auto n : f->get_ordered_ops()) {
        if (jitters.find(n->get_type_info()) != jitters.end()) {
            lowered.push_back(std::make_pair(jitters[n->get_type_info()](n), ngraph::snippets::getRegisters(n)));
        } else {
    ...
    for (auto n : f_scalar->get_ordered_ops()) {
        if (jitters.find(n->get_type_info()) != jitters.end()) {
            scalar_lowered.push_back(make_pair(jitters[n->get_type_info()](n), ngraph::snippets::getRegisters(n)));
        } else {
            throw ngraph::ngraph_error(std::string("unknown operation ") + n->get_type_info().name);
        }
    }
    ...
    std::cout << "kernel is emited" << std::endl;
    lowered.insert(lowered.end(), scalar_lowered.begin(), scalar_lowered.end());
    for (auto& op : lowered) {
        op.first->emit_data();
    }
    std::cout << "constants are emitted" << std::endl;
    h->create_kernel();
    return h->jit_ker();
}
