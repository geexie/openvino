...
static void printPerformanceCounts(InferenceEngine::InferRequest request, std::ostream &stream, std::string deviceName, bool bshowHeader = true) {
...
void LayerTestsCommon::Infer() {
    ...
    // FIXME: add option to test setup
    // for (int i = 0; i < 100; i++)
    //     inferRequest.Infer();

    // printPerformanceCounts(inferRequest, std::cout, "CPU0", false);
}
